...
PopulationAlgorithmAdvanced::PopulationAlgorithmAdvanced(PopulationStateAdvanced&  popState,
                                                         GslRandomNumberGenerator& rng, bool parallel)
    : Algorithm(popState, rng), m_popState(popState)
{
        m_init           = false;
...
bool_t PopulationAlgorithmAdvanced::init()
{
        ...
        if (m_parallel) {
#ifndef DISABLEOPENMP
                std::cerr << "# PopulationAlgorithmAdvanced: using parallel version with " << omp_get_max_threads()
                          << " threads" << std::endl;
