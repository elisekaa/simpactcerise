...
CoarseMap::CoarseMap(int subDivX, int subDivY)
    : m_subDivX(subDivX), m_subDivY(subDivY), m_minX(numeric_limits<double>::infinity()),
      m_maxX(-numeric_limits<double>::infinity()), m_minY(numeric_limits<double>::infinity()),
      m_maxY(-numeric_limits<double>::infinity()), m_cellWidth(1), m_cellHeight(1)

{
        assert(subDivX > 0 && subDivY > 0);
}

CoarseMap::~CoarseMap() { clearGrid(m_cells); }
...
CoarseMapCell* CoarseMap::getCell(Person* pPerson, bool canRearrange)
{
        ...
        if (location.x < m_maxX && location.x >= m_minX && location.y < m_maxY && location.y >= m_minY) {
                ...
                if (location.x < m_minX)
                        m_minX = location.x;
                if (location.x > m_maxX)
                        m_maxX = location.x;
                if (location.y < m_minY)
                        m_minY = location.y;
                if (location.y > m_maxY)
                        m_maxY = location.y;
...
void CoarseMap::addPerson(Person* pPerson) { addPersonInternal(pPerson, true); }
