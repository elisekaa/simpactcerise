...
EventAIDSStage::EventAIDSStage(Person* pPerson, bool final) : SimpactEvent(pPerson) { m_finalStage = final; }
...
double EventAIDSStage::getNewStageTime(double currentTime) const
{
        ...
        if (newStageTime <= currentTime) {
                ...
                //		cerr << "Warning: advancing faster to new AIDS stage for person " << pPerson->getName()
                //<< endl;
        }
