...
void EventSeedBase::fire(SeedEventSettings& settings, double t, State* pState, TransmissionFunction infectPerson)
{
        ...
        assert(settings.m_seedGender == SeedEventSettings::Any || settings.m_seedGender == SeedEventSettings::Male ||
        ...
        if (!settings.m_useFraction && settings.m_stopOnShort && countSeeded != settings.m_seedAmount)
                abortWithMessage(
                    strprintf("Could not HSV2 seed the requested amount of people: %d were seeded, but %d requested",
                              countSeeded, settings.m_seedAmount));
}

void EventSeedBase::processConfig(SeedEventSettings& settings, ConfigSettings& config,
                                  GslRandomNumberGenerator* pRndGen, const string& configName)
{
