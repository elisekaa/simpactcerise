...
private:
        ...
        {
                return m_helper.calculateInternalTimeInterval(pState, t0, dt, this);
        }
        double solveForRealTimeInterval(const State* pState, double Tdiff, double t0)
        {
                return m_helper.solveForRealTimeInterval(pState, Tdiff, t0, this);
        }
...
inline double EventVariableFireTime_Helper::getNewInternalTimeDifference(GslRandomNumberGenerator* pRndGen,
                                                                         const State*              pState)
{
        assert(m_fireTime > 0);
        assert(m_alpha < 0);
