...
#include "event/eventcheckstopalgorithm.h"
#include "event/eventchronicstage.h"
#include "event/eventdebut.h"
#include "event/eventformation.h"
#include "event/eventhivseed.h"
#include "event/eventhsv2seed.h"
#include "event/eventintervention.h"
#include "event/eventmortality.h"
#include "event/eventperiodiclogging.h"
#include "event/eventrelocation.h"
#include "event/eventsyncpopstats.h"
#include "event/eventsyncrefyear.h"
                                                  ...
                                                  const PopulationDistribution&  popDist)
{
        ...
        FixedValueDistribution2D* pDist2D =
            dynamic_cast<FixedValueDistribution2D*>(Person::getPopulationDistribution());
        if (pDist2D ==
            0) // For the fixed value distribution, we'll use the old behaviour, but otherwise the course map is used
        {
...
bool_t SimpactPopulation::scheduleInitialEvents()
{
        ...
        if (m_msm) {
                for (int i = 0; i < numMen; i++) {
                        ...
                        if (pMan->isSexuallyActive())
                                initializeFormationEvents(
                                    pMan, true, false,
                                    0); // Only do MSM stuff, relationships with women are already done
                }
        }
        ...
        if (EventSyncPopulationStatistics::isEnabled()) {
                EventSyncPopulationStatistics* pEvt =
                    new EventSyncPopulationStatistics(); // global event, recalcs everything
                onNewEvent(pEvt);
        }
...
void SimpactPopulation::initializeFormationEvents(Person* pPerson, bool initializationPhase, bool relocation,
                                                  double tNow)
{
        ...
        } else // Use eyecaps
        {
                ...
                if (pPerson->getGender() == Person::Male) {
                        ...
                        for (int i = 0; i < numInterests; i++) {
                                ...
                                if (pPoi->isMan() &&
                                    pPoi->getPersonID() <
                                        pMan->getPersonID()) // MSM, order persons TODO: is this really necessary?
