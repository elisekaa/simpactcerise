...
public:
        ...
        int getLastKnownPopulationSize(double& popTime) const
        {
                popTime = m_lastKnownPopulationSizeTime;
                assert(popTime >= 0);
                assert(m_lastKnownPopulationSize >= 0);
                return m_lastKnownPopulationSize;
        }
        void setLastKnownPopulationSize();

        double getReferenceYear() const { return m_referenceYear; }
        void   setReferenceYear(double t)
        {
                assert(t >= 0);
                m_referenceYear = t;
        }
