...
BinormalDistribution::BinormalDistribution(double mu, double sigma, double rho, GslRandomNumberGenerator* pRndGen,
    ...
    : ProbabilityDistribution2D(pRndGen, true)
{
        if (!(minVal < maxVal))
...
BinormalDistribution::BinormalDistribution(double muX, double muY, double sigmaX, double sigmaY, double rho,
                                           GslRandomNumberGenerator* pRndGen, double minValX, double maxValX,
...
Point2D BinormalDistribution::pickPoint() const
{
        ...
        while ((xy.first < m_minX || xy.first > m_maxX || xy.second < m_minY || xy.second > m_maxY) &&
               count++ < maxCount)
                xy = pRndGen->pickBivariateGaussian(m_muX, m_muY, m_sigmaX, m_sigmaY, m_rho);

        if (count >= maxCount)
                abortWithMessage("BinormalDistribution::pickPoint: rejection sampling couldn't find a point within the "
                                 "specified region");

        return Point2D(xy.first, xy.second);
}

double BinormalDistribution::pickMarginalX() const { return pickClippedGaussian(m_muX, m_sigmaX, m_minX, m_maxX); }

double BinormalDistribution::pickMarginalY() const { return pickClippedGaussian(m_muY, m_sigmaY, m_minY, m_maxY); }
...
double BinormalDistribution::pickClippedGaussian(double mean, double sigma, double minVal, double maxVal) const
{
        ...
        if (count >= maxCount)
                abortWithMessage("BinormalDistribution::pickClippedGaussian: rejection sampling couldn't find a point "
...
double BinormalDistribution::pickConditional(double val, double meanDest, double meanCond, double sigmaDest,
                                             double sigmaCond, double minVal, double maxVal) const
{
