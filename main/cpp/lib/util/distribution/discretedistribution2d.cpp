...
DiscreteDistribution2D::DiscreteDistribution2D(double xOffset, double yOffset, double xSize, double ySize,
                                               ...
                                               const Polygon2D& filter)
    : ProbabilityDistribution2D(pRngGen, true)
{
        m_pMarginalXDist = 0;
        m_pMarginalYDist = 0;
        m_xSize          = xSize;
        ...
        generateConditionalsAndMarginal(xOffset, yOffset, xSize, ySize, density, pRngGen, filter, false,
                                        m_conditionalXDists, &m_pMarginalYDist);
        generateConditionalsAndMarginal(xOffset, yOffset, xSize, ySize, density, pRngGen, filter, true,
                                        m_conditionalYDists, &m_pMarginalXDist);
...
void DiscreteDistribution2D::generateConditionalsAndMarginal(double xOffset, double yOffset, double xSize, double ySize,
                                                             const GridValues&         density,
                                                             GslRandomNumberGenerator* pRngGen, const Polygon2D& filter,
                                                             bool transpose,
#ifdef OLDTEST
...
)
{
        ...
        if (!hasValue)
                abortWithMessage("No non-zero value found in DiscreteDistribution2D::generateConditionalsAndMarginal. "
                                 "Bad data file.");
