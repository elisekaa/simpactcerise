...
bool_t DiscreteDistributionWrapper2D::init(const std::string& densFile, const std::string& maskFile, double xOffset,
                                           double yOffset, double width, double height, bool flipY, bool floor)
{
        ...
        if (pMask) {
                ...
                if (!(pMask->getWidth() == w && pMask->getHeight() == h))
                        return "Dimensions of density file '" + densFile + "' and mask file '" + maskFile +
                               "' don't match";
