...
DebugTimerMap::~DebugTimerMap()
{
        ...
        for (auto it = m_timers.begin(); it != end; ++it) {
                ...
                cerr << it->first << "\tit: " << pTimer->getIterations()
                     << "\tduration: " << duration_cast<nanoseconds>(diff).count() << " ns | "
                     << duration_cast<microseconds>(diff).count() << " us | "
                     << duration_cast<milliseconds>(diff).count() << " ms | " << duration_cast<seconds>(diff).count()
                     << " s" << endl;
        }
}
