...
GslRandomNumberGenerator::~GslRandomNumberGenerator() { gsl_rng_free(m_pRng); }
...
unsigned int GslRandomNumberGenerator::pickPoissonNumber(double lambda) { return gsl_ran_poisson(m_pRng, lambda); }
...
{
        if (ageMin < 0) {
                std::cerr << "WARNING: GslRandomNumberGenerator::pickWeibull -> ageMin should be at least 0 but is "
                          << ageMin << ". Clipping it." << std::endl;
                ageMin = 0;
        }
...
std::pair<double, double> GslRandomNumberGenerator::pickBivariateGaussian(double muX, double muY, double sigmaX,
