...
inline double EventBase::solveForRealTimeInterval(const State* pState, double t0)
{
        ...
        if (s_checkInverse) {
                ...
                if (diff > 1e-8) {
                        std::cerr
                            << "ERROR: mismatch 1 between solveForRealTimeInterval and calculateInternalTimeInterval"
                            << std::endl;
                        abort();
                }
        }
...
inline void EventBase::subtractInternalTimeInterval(const State* pState, double t1)
{
        ...
        if (s_checkInverse) {
                ...
                if (diff > 1e-8) {
                        std::cerr
                            << "ERROR: mismatch 2 between solveForRealTimeInterval and calculateInternalTimeInterval"
                            << std::endl;
                        abort();
                }
        }
