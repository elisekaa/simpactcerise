...
public:
        ...
        void setSexuallyActive(double t)
        {
                m_sexuallyActive = true;
                assert(t >= 0);
                m_debutTime = t;
        }
        ...
        double getPreferredAgeDifference() const
        {
                assert(m_preferredAgeDiffHetero < 200.0 && m_preferredAgeDiffHetero > -200.0);
                return m_preferredAgeDiffHetero;
        }

        double getFormationEagernessParameterMSM() const { return m_formationEagernessHomo; }
        double getPreferredAgeDifferenceMSM() const
        {
                assert(m_preferredAgeDiffHomo < 200.0 && m_preferredAgeDiffHomo > -200.0);
                return m_preferredAgeDiffHomo;
        }
        ...
        Person* getPersonOfInterest(int idx) const
        {
                assert(idx >= 0 && idx < (int)m_personsOfInterest.size());
                Person* pPerson = m_personsOfInterest[idx];
                assert(pPerson);
                return pPerson;
        }

        static void writeToRelationLog(const Person* pMan, const Person* pWoman, double formationTime,
...
private:
        ...
        public:
                ...
                Relationship(Person* pPerson, double formationTime)
                {
                        assert(pPerson != 0);
                        assert(formationTime > 0);
                        m_pPerson       = pPerson;
                        m_formationTime = formationTime;
                }
                Relationship(Person* pPerson)
                {
                        assert(pPerson != 0);
                        m_pPerson       = pPerson;
                        m_formationTime = -1;
        ...
        struct EagernessAndAgegap
        {
                ...
                void obtainConfig(ConfigWriter& config, const std::string& prefixEag, const std::string& prefixGap,
                                  const std::string& homSuff);
        };
