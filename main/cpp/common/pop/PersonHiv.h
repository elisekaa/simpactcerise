...
public:
        ...
        double getSetPointViralLoad() const
        {
                assert(m_infectionStage != NoInfection);
                return m_Vsp;
        }
        double getOriginalViralLoad() const
        {
                assert(isInfected());
                assert(m_VspOriginal > 0);
                return m_VspOriginal;
        }
        double getViralLoad() const;
        void   lowerViralLoad(double fractionOnLogscale, double treatmentTime);
        bool   hasLoweredViralLoad() const
        {
                assert(isInfected());
                assert(m_Vsp > 0);
                return m_VspLowered;
        }
        double getLastTreatmentStartTime() const
        {
                assert(isInfected());
                assert(m_Vsp > 0);
                assert(m_VspLowered);
        ...
        int  getNumberTreatmentStarted() const
        {
                assert(isInfected());
        ...
        double getLastCD4CountAtARTStart() const
        {
                assert(isInfected());
                assert(m_VspLowered);
                return m_lastCD4AtTreatmentStart;
        }
