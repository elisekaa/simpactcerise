...
void Person_Relations::removeRelationship(Person* pPerson, double t, bool deathBased)
{
        ...
        if (it == m_relationshipsSet.end())
                abortWithMessage(strprintf("Consistency error: a person was not found exactly once in the relationship "
                                           "list (this = %s, person = %s)",
                                           m_pSelf->getName().c_str(), pPerson->getName().c_str()));
        ...
        if (deathBased)
                ...
                if (m_pSelf->isMan() && relation.getPartner()->isWoman())
                        writeToLog = true;
                else if (m_pSelf->isMan() && relation.getPartner()->isMan() &&
                         m_pSelf->getPersonID() < relation.getPartner()->getPersonID())
                        writeToLog = true;
        }
...
void Person_Relations::removePersonOfInterest(Person* pPerson)
{
        ...
        abortWithMessage("Specified person of interest " + pPerson->getName() + " was not found in list of " +
                         m_pSelf->getName());
}
                                          ...
                                          double dissolutionTime)
{
        ...
        LogRelation.print("%d,%d,%10.10f,%10.10f,%10.10f,%d", (int)pMan->getPersonID(),
                          (int)pWomanOrMan2->getPersonID(), formationTime, dissolutionTime,
                          pWomanOrMan2->getDateOfBirth() - pMan->getDateOfBirth(),
