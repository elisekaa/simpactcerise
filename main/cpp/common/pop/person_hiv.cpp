...
void Person_HIV::writeToViralLoadLog(double tNow, const string& description) const
{
        ...
        LogViralLoadHIV.print("%10.10f,%d,%s,%10.10f,%10.10f", tNow, id, description.c_str(), log10(m_Vsp),
                              log10(currentVl));
}
...
void Person_HIV::processConfig(ConfigSettings& config, GslRandomNumberGenerator* pRndGen)
{
        ...
        if (!(r = config.getKeyValue("person.vsp.model.type", VspModelName, supportedModels)) ||
            ...
            !(r = config.getKeyValue("person.vsp.tofinalaids.x", m_finalAidsFromSetPointParamX,
                                     m_aidsFromSetPointParamX)) ||
        ...
        if (VspModelName == "logweibullwithnoise") {
                ...
                if (useAltSeedDist)
                        pAltSeedDist =
                            getDistributionFromConfig(config, pRndGen, "person.vsp.model.logdist2d.alternativeseed");
...

        {
                ...
                if ((pDist = dynamic_cast<VspModelLogWeibullWithRandomNoise*>(m_pVspModel)) != 0) {
                        ...
                        if (!(r = config.addKey("person.vsp.model.type", "logweibullwithnoise")) ||
                            !(r = config.addKey("person.vsp.model.logweibullwithnoise.weibullshape",
                                                pDist->getWeibullShape())) ||
                            !(r = config.addKey("person.vsp.model.logweibullwithnoise.weibullscale",
                                                pDist->getWeibullScale())) ||
                            !(r = config.addKey("person.vsp.model.logweibullwithnoise.fracsigma",
                                                pDist->getSigmaFraction())))
                                abortWithMessage(r.getErrorString());
        ...
        }
        {
                ...
                if ((pDist = dynamic_cast<VspModelLogDist*>(m_pVspModel)) != 0) {
                        ...
                        if (!(r = config.addKey("person.vsp.model.type", "logdist2d")) ||
                            !(r = config.addKey("person.vsp.model.logdist2d.usealternativeseeddist",
                                                (pAltSeedDist != 0))))
                                abortWithMessage(r.getErrorString());

                        if (pAltSeedDist)
                                addDistributionToConfig(pAltSeedDist, config,
                                                        "person.vsp.model.logdist2d.alternativeseed");
