        ...
        if (m_configLog.size() == 0) // first entry
        {
                ...
                for (size_t k = 0; k < keys.size(); k++) {
                        ...
                        if (!config.getStringKeyValue(key, value, used))
                                abortWithMessage("Unexpected error: couldn't retrieve config value for key '" + key +
                                                 "' even though it was reported to exist");
        ...
        for (size_t i = 0; i < numEntries; i++) {
                ...
                for (auto it = keys.begin(); it != keys.end(); ++it) {
                        ...
                        if (vals.size() != numEntries)
                                abortWithMessage(strprintf("key %s, numEntries: %d vals.size(): %d", it->c_str(),
                                                           (int)numEntries, (int)vals.size()));
