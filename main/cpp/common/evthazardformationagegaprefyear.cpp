...
EvtHazardFormationAgeGapRefYear::EvtHazardFormationAgeGapRefYear(const string& hazName, bool msm, double a0, double a1,
                                                                 double a2, double a3, double a4, double a6, double a7,
                                                                 double a8, double a10, double aDist, double agfmConst,
                                                                 double agfmExp, double agfmAge, double agfwConst,
                                                                 double agfwExp, double agfwAge, double numRelScaleMan,
                                                                 double numRelScaleWoman, double b, double tMax,
...
double EvtHazardFormationAgeGapRefYear::getA0(const SimpactPopulation& population, Person* pPerson1, Person* pPerson2)
{
        ...
        double a0_total =
            a0_base - std::log((n / 2.0) * eyeCapsFraction); // log(x/(n/2)) = log(x) - log(n/2) = a0_base - log(n/2)

        return a0_total;
}

double EvtHazardFormationAgeGapRefYear::getTr(const SimpactPopulation& population, Person* pPerson1, Person* pPerson2,
                                              double t0, double lastDissTime)
{
        ...
        if (tr < 0) // did not have a relationship before, use
        {
                ...
                assert(
                    t0 - tr >
                    -1e-10); // something slightly negative is possible due to finite precision and error accumulation
        }

        return tr;
}

EvtHazard* EvtHazardFormationAgeGapRefYear::processConfig(ConfigSettings& config, const string& prefix,
                                                          const string& hazName, bool msm)
{
        ...
        return new EvtHazardFormationAgeGapRefYear(hazName, msm, a0, a1, a2, a3, a4, a6, a7, a8, a10, aDist, agfmConst,
                                                   agfmExp, agfmAge, agfwConst, agfwExp, agfwAge, numRelScaleMan,
                                                   numRelScaleWoman, b, tMax, tMaxAgeRefDiff);
}
