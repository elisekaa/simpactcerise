...
bool areValuesCompatible(const string& key, const std::string& A, const std::string& B,
                         bool canIgnore = true) // B can be 'IGNORE', just print a warning then
{
        if (canIgnore) {
                if (B == "IGNORE") {
                        cerr << "# WARNING: ignoring consistency check for config key " << key << " (config value is '"
                             << A << "')" << endl;
                        return true;
                }
        }
        ...
        if (parseAsDouble(A, x1) && parseAsDouble(B, x2)) {
                ...
                if (diff < 1e-10) {
                        cerr << "# WARNING: ignoring small (" << diff << ") difference between " << A << " and " << B
                             << " in key " << key << endl;
                        return true;
                }
                cerr << "# ERROR: relative difference between two doubles (" << A << " and " << B
                     << ") is too large: " << diff << endl;
                return false;
        }
                        ...
                        double tMax, int64_t maxEvents)
{
        ...
        for (size_t i = 0; i < keys.size(); i++) {
                ...
                if (!config.getKeyValue(keys[i], val2))
                        abortWithMessage("Consistency error: " + keys[i] +
                                         " is present in config file but doesn't seem to be configured");

                // cerr << "Checking " << keys[i] << ":" << val1 << " vs " << val2 << endl;
                if (!areValuesCompatible(keys[i], val1, val2))
                        abortWithMessage("Consistency error: inconsistency for key " + keys[i] + " (" + val1 + " <-> " +
                                         val2 + ")");
        }
        ...
        for (size_t i = 0; i < keys.size(); i++) {
                ...
                if (!loadedConfig.getStringKeyValue(keys[i], val1, dummy))
                        abortWithMessage("Consistency error: " + keys[i] +
                                         " is configured but not present in config file");
