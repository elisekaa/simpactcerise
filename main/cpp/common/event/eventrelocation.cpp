...
string EventRelocation::getDescription(double tNow) const { return "relocation"; }
...
void EventRelocation::fire(Algorithm* pAlgorithm, State* pState, double t)
{
        ...
        if (pPerson->isSexuallyActive() && pPerson->hiv().getInfectionStage() != PersonHIV::AIDSFinal) {
                // if eyecaps < 1.0 a new set of persons of interest should be chosen
                population.initializeFormationEvents(pPerson, false, true,
                                                     t); // true: due to a relocation, does the eyecaps check if needed
        }
...
ConfigFunctions relocationConfigFunctions(EventRelocation::processConfig, EventRelocation::obtainConfig,
                                          "EventRelocation");
