...
EventFormation::EventFormation(Person* pPerson1, Person* pPerson2, double lastDissTime, double formationScheduleTime)
    : SimpactEvent(pPerson1, pPerson2), m_lastDissolutionTime(lastDissTime),
...
string EventFormation::getDescription(double tNow) const
{
        ...
        return strprintf("%s between %s and %s", evtName.c_str(), getPerson(0)->getName().c_str(),
...
void EventFormation::fire(Algorithm* pAlgorithm, State* pState, double t)
{
        ...
        if (pPerson2->isWoman()) {
                ...
                if (!pWoman->isPregnant()) {
                        EventConception* pEvtConception =
                            new EventConception(pPerson1, pPerson2, t); // should be: man, woman, formation time
