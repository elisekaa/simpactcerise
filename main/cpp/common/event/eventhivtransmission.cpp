...
{
        return strprintf("Transmission event from %s to %s", getPerson(0)->getName().c_str(),
...
double EventHIVTransmission::calculateHazardFactor(const SimpactPopulation& population, double t0)
{
        ...
        double logh = s_a + s_b * std::pow(V, -s_c) + s_d1 * Pi + s_d2 * Pj + s_e1 * getH(pPerson1) +
                      s_e2 * getH(pPerson2) + s_g1 * pPerson2->hiv().getHazardB0Parameter() +
                      s_g2 * pPerson2->hiv().getHazardB1Parameter();
