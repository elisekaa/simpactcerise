...
double EventIntervention::getNewInternalTimeDifference(GslRandomNumberGenerator* pRndGen, const State* pState)
{
        ...
        if (evtTime < 0 || dt < 0)
                abortWithMessage(
                    "EventIntervention::getNextInterventionTime: the next intervention takes place at time " +
                    doubleToString(evtTime) + " which is before the current time " +
                    doubleToString(population.getTime()) + " (dt = " + doubleToString(dt) + ")");

        return dt;
}

string EventIntervention::getDescription(double tNow) const { return "Intervention event"; }
