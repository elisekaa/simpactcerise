...
string EventAIDSMortality::getDescription(double tNow) const
{
        ...
        return strprintf("AIDS death of %s (current age %g, in treatment: %d)", pPerson->getName().c_str(),
                         pPerson->getAgeAt(tNow), (int)pPerson->hiv().hasLoweredViralLoad());
}
...
ConfigFunctions aidsMortalityConfigFunctions(EventAIDSMortality::processConfig, EventAIDSMortality::obtainConfig,
                                             "EventAIDSMortality");
