...
void EventDiagnosis::obtainConfig(ConfigWriter& config)
{
        ...
        if (!(r = config.addKey("diagnosis.baseline", s_baseline)) ||
            ...
            !(r = config.addKey("diagnosis.beta", s_beta)) || !(r = config.addKey("diagnosis.t_max", s_tMax)) ||
...
HazardFunctionDiagnosis::HazardFunctionDiagnosis(Person* pPerson, double baseline, double ageFactor,
    ...
    : m_baseline(baseline), m_ageFactor(ageFactor), m_genderFactor(genderFactor),
      m_diagPartnersFactor(diagPartnersFactor), m_isDiagnosedFactor(isDiagnosedFactor), m_beta(beta),
      m_HSV2factor(HSV2factor)
{
        assert(pPerson != 0);
        m_pPerson = pPerson;
        ...
        double A = baseline - ageFactor * tb + genderFactor * G + diagPartnersFactor * D +
                   isDiagnosedFactor * hasBeenDiagnosed - beta * tinf + HSV2factor * HSV2;
