...
EventDissolution::EventDissolution(Person* pPerson1, Person* pPerson2, double formationTime)
...
std::string EventDissolution::getDescription(double tNow) const
{
        ...
        return strprintf("%s between %s and %s, relationship was formed at %g (%g ago)", evtName.c_str(),
                         getPerson(0)->getName().c_str(), getPerson(1)->getName().c_str(), m_formationTime,
                         tNow - m_formationTime);
}
...
ConfigFunctions dissolutionConfigFunctions(EventDissolution::processConfig, EventDissolution::obtainConfig,
                                           "EventDissolution");
