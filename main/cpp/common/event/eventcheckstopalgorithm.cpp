...
string EventCheckStopAlgorithm::getDescription(double tNow) const { return "checkstopalgorithm"; }
...
void EventCheckStopAlgorithm::fire(Algorithm* pAlgorithm, State* pState, double t)
{
        ...
        if (popSize > s_maxPopSize)
                pState->setAbortAlgorithm(
                    strprintf("Check failed (simulation time = %g): Population size %d exceeds specified maximum %g", t,
                              popSize, s_maxPopSize));
        if (curTime - m_startTime > s_maxRunningTime)
                pState->setAbortAlgorithm(strprintf("Check failed (simulation time = %g): Maximum running time (real "
                                                    "time, not simulation time) of %g seconds is exceeded",
                                                    t, s_maxRunningTime));

        // cout << "# curTime = " << curTime << " m_startTime = " << m_startTime << " s_maxRunningTime = " <<
        // s_maxRunningTime << endl;
...
void EventCheckStopAlgorithm::processConfig(ConfigSettings& config, GslRandomNumberGenerator* pRndGen)
{
        ...
        if (isEnabled()) {
                ...
                if (s_maxRunningTime <= 0)
                        abortWithMessage(
                            "Specified running time (wallclock time, not simulation time!) must be positive");
        }
}
