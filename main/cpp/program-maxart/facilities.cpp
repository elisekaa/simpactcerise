...
double toRad(double deg) { return deg / 180.0 * M_PI; }
...
{
        switch (t) {
        case Facility::ControlStage: return "ControlStage";
        case Facility::TransitionStage: return "TransitionStage";
        case Facility::InterventionStage: return "InterventionStage";
        default: break;
        }
        return "UnknownStage";
}
...
void Facilities::dump()
{
        ...
        for (size_t i = 0; i < m_facilities.size(); i++)
                cout << m_facilities[i].getName() << "," << m_facilities[i].getPosition().x << ","
                     << m_facilities[i].getPosition().y << "," << getStageName(m_facilities[i].getStage()) << endl;
...
void Facilities::processConfig(ConfigSettings& config, GslRandomNumberGenerator* pRndGen)
{
        ...
        // Read the file with the GPS coords
        {
                ...
                while (ReadInputLine(pFile, line)) {
                        ...
                        if (!parseAsDouble(parts[1], y) || !parseAsDouble(parts[2], x))
                                abortWithMessage("Can't interpret '" + parts[1] + "' or '" + parts[2] +
                                                 "' as a number in file " + coordsFile);

                        if (x < -180.0 || x > 180.0 || y < -90.0 || y > 90.0)
                                abortWithMessage("Can't interpret '" + parts[1] + "' or '" + parts[2] +
                                                 "' as a valid longitude and latitude in file " + coordsFile);

                        // TODO: what are reasonable limits for the approximation to work?
                        if (std::abs(y - s_startLattitude) > 3.0 ||
                            std::abs(x - s_startLongitude) > 3.0 * std::cos(toRad(s_startLattitude)))
                                abortWithMessage(
                                    "Coordinates '" + parts[1] + "' or '" + parts[2] +
                                    "' lie too far from start coordinates for flat approximation to work in file " +
                                    coordsFile);
        ...
        // Read the file with the randomization
        {
                ...
                while (ReadInputLine(pFile, line)) {
                        ...
                        if (!parseAsInt(parts[0], step))
                                abortWithMessage("Can't interpret '" + parts[0] + "' as a step number in file '" +
                                                 randFile + "'");
                        ...
                        if (facilities.size() == 0)
                                abortWithMessage("No facilities present for step number " + intToString(step) +
                                                 " in file '" + randFile + "'");
                        ...
                        for (size_t i = 0; i < facilities.size(); i++) {
                                ...
                                if (facility.length() == 0)
                                        abortWithMessage("Empty facility name for step step number " +
                                                         intToString(step) + " in file '" + randFile + "'");

                                if (randomization.find(facility) != randomization.end())
                                        abortWithMessage("Facility '" + facility + "' occurs at least twice in file '" +
                                                         randFile + "'");
        ...
        // Perform checks on randomization and geo info
        {
                ...
                for (auto it = facilityCoords.begin(); it != facilityCoords.end(); it++) {
                        ...
                        if (randomization.find(name) == randomization.end())
                                abortWithMessage("Facility '" + name +
                                                 "' is mentioned in coords file, but is not present in randomization");
                }
                ...
                for (auto it = randomization.begin(); it != randomization.end(); it++) {
                        ...
                        if (facilityCoords.find(name) == facilityCoords.end())
                                abortWithMessage(
                                    "Facility '" + name +
                                    "' is mentioned in randomization file, but is not present in coords file");
                ...
                for (int s = minStep; s <= maxStep; s++) {
                        if (stepMap.find(s) == stepMap.end())
                                abortWithMessage("Expecting step '" + intToString(s) +
                                                 "' but it doesn't appear to be present in randomization file");
                }
        }
        ...
        if (s_coordOutfile.length() > 0) {
                ...
                if (!(r = coordLog.open(s_coordOutfile)))
                        abortWithMessage("Can't write facility XY positions to '" + s_coordOutfile +
                                         "':" + r.getErrorString());
