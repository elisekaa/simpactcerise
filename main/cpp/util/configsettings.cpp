...
bool_t ConfigSettings::getKeyValue(const std::string& key, std::string& value, const vector<string>& allowedValues)
{
        ...
        if (allowedValues.size() > 0) {
                ...
                if (!found) {
                        ...
                        extraInfo += ")";
                        return "Specified value '" + value + "' for key '" + key + "' is not an allowed value " +
                               extraInfo + ".";
                }
        }
...
bool_t ConfigSettings::getKeyValue(const std::string& key, double& value, double minValue, double maxValue)
{
        ...
        if (value < minValue || value > maxValue)
                return strprintf("The value for '%s' must lie between %g and %g, but is %g", key.c_str(), minValue,
                                 maxValue, value);
...
bool_t ConfigSettings::getKeyValue(const std::string& key, int& value, int minValue, int maxValue)
{
        ...
        if (value < minValue || value > maxValue)
                return strprintf("The value for '%s' must lie between %d and %d, but is %d", key.c_str(), minValue,
                                 maxValue, value);
...
bool_t ConfigSettings::getKeyValue(const std::string& key, int64_t& value, int64_t minValue, int64_t maxValue)
{
        ...
        if (value < minValue || value > maxValue)
                return strprintf("The value for '%s' must lie between %" PRId64 " and %" PRId64 ", but is %" PRId64 "",
...
bool_t ConfigSettings::getKeyValue(const string& key, vector<double>& values, double minValue, double maxValue)
{
        ...
        if (!parseAsDoubleVector(valueStr, values, badField))
                return "Can't interpret value for key '" + key + "' as a list of floating point numbers (field '" +
                       badField + "' is bad)";

        for (size_t i = 0; i < values.size(); i++) {
                double value = values[i];
                if (value < minValue || value > maxValue)
                        return strprintf("Each value for '%s' must lie between %g and %g, but one is %g", key.c_str(),
