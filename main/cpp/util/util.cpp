...
void SplitLine(const string& line, vector<string>& args, const string& separatorChars, const string& quoteChars,
               const string& commentStartChars, bool ignoreZeroLengthFields)
{
        ...
        while (startPos < line.length() && !done) {
                ...
                while (!endFound && endPos < line.length()) {
                        if (HasCharacter(separatorChars, line[endPos]) ||
                            HasCharacter(commentStartChars, line[endPos])) {
