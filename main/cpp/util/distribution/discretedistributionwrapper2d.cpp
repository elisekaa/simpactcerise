...
DiscreteDistributionWrapper2D::DiscreteDistributionWrapper2D(GslRandomNumberGenerator* pRndGen)
                                               ...
                                               double yOffset, double width, double height, bool flipY, bool floor)
{
        ...
        if (pDens == 0)
                return ExitStatus("Unable to process '" + densFile + "': " + r.getErrorString());
        ...
        if (maskFile.length() > 0) {
                ...
                if (pMask == 0)
                        return ExitStatus("Unable to process '" + maskFile + "': " + r.getErrorString());
        }
        ...
        if (!(r = pDens->init(densFile, true, flipY)))
                return ExitStatus("Unable to load density file '" + densFile + "': " + r.getErrorString());
        if (pMask) {
                if (!(r = pMask->init(maskFile, false, flipY)))
                        return ExitStatus("Unable to load mask file '" + maskFile + "': " + r.getErrorString());
        ...
        m_pDist =
            new DiscreteDistribution2D(xOffset, yOffset, width, height, *pDens, floor, getRandomNumberGenerator());
...
ExitStatus DiscreteDistributionWrapper2D::allocateGridFunction(const std::string& fileName, GridValues** pGf)
{
        ...
        return ExitStatus("Can't determine file reader based on extension (only TIFF and CSV are supported)");
}
