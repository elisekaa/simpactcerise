 ...
 *
 *  The probability density is the following: \f[ \textrm{prob}(x) = \frac{1}{x s \sqrt{2 \pi} }
 * \exp\left(-\frac{(\textrm{ln}(x)-z)^2}{2 s^2}\right) \f]
 */
...
inline LogNormalDistribution::LogNormalDistribution(double zeta, double sigma, GslRandomNumberGenerator* pRng)
    : ProbabilityDistribution(pRng)
{
        m_zeta  = zeta;
        m_sigma = sigma;
}
