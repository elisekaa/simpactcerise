 ...
 *
 *  The probability density is the following: \f[ \textrm{prob}(x) = \frac{ x^{a-1} \exp\left(-\frac{x}{b} \right) }
 * {b^a \Gamma(a) } \f]
 */
...
public:
        ...
        GammaDistribution(double a, double b, GslRandomNumberGenerator* pRng) : ProbabilityDistribution(pRng)
        {
                m_a = a;
                m_b = b;
        }
