...
ExitStatus DiscreteDistributionWrapper::init(const std::string& csvFileName, double xMin, double xMax, int yCol, bool floor)
{
        ...
        if (yCol < 1 || yCol > csv.getNumberOfColumns())
                return ExitStatus("Specified column number is invalid");

        if (!(xMin < xMax))
                return ExitStatus("Minimum x-value must be smaller than the maximum x-value");

        if (!(xMin > -numeric_limits<double>::infinity() && xMax < numeric_limits<double>::infinity()))
                return ExitStatus("Both minimum and maximum value must be real numbers (not infinity)");

        const int numRows = csv.getNumberOfRows();
        if (numRows < 1)
                return ExitStatus("CSV File does not contain any data");
...
ExitStatus DiscreteDistributionWrapper::init(const std::string& csvFileName, int xCol, int yCol, bool floor)
{
        if (m_pDist)
                return ExitStatus("Already initialized");
        ...
        if (xCol < 1 || xCol > csv.getNumberOfColumns())
                return ExitStatus("Specified y column number is invalid");
        ...
        const int numRows = csv.getNumberOfRows();
        if (numRows < 2)
                return ExitStatus("Number of rows in CSV file must be at least two");
        ...
        for (int y = 0; y < numRows; y++) {
                ...
                if (!(xValues[y] > -numeric_limits<double>::infinity() &&
                      xValues[y] < numeric_limits<double>::infinity()))
                        return ExitStatus("The x-entry at row " + intToString(y + 1) + " is not a real value");
        ...
        if (yValues[numRows - 1] != 0)
                return ExitStatus("The final y value must be zero");
                                         ...
                                         bool floor)
{
        ...
        if (xValues.size() != yValues.size())
                return ExitStatus("Number of x-values must equal the number of y-values");

        if (xValues.size() < 2)
                return ExitStatus("At least two x-values must be specified");

        const int num = (int)yValues.size();
        if (yValues[num - 1] != 0)
                return ExitStatus("The last y value must be zero");
        ...
        for (int i = 0; i < num; i++) {
                ...
                if (!(xValues[i] > -numeric_limits<double>::infinity() &&
                      xValues[i] < numeric_limits<double>::infinity()))
                        return ExitStatus("The x-entry at position " + intToString(i + 1) + " is not a real value");
