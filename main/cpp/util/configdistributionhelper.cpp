...
#include "csvfile.h"
#include "distribution/betadistribution.h"
#include "distribution/binormaldistribution.h"
#include "distribution/discretedistribution2d.h"
#include "distribution/discretedistributionwrapper.h"
#include "distribution/discretedistributionwrapper2d.h"
#include "distribution/exponentialdistribution.h"
#include "distribution/fixedvaluedistribution.h"
...
#include "distribution/uniformdistribution2d.h"
                                                   ...
                                                   const string& prefix)
{
        ...
        if (distName == "fixed") {
                ...
                if (!(r = pDist0->init(fileName, xMin, xMax, yCol, floor)))
                        abortWithMessage("Unable to initialize 1D distribution for " + prefix + ": " +
                                         r.getErrorString());
                ...
                if ((pDist = dynamic_cast<DiscreteDistributionWrapper*>(pSrcDist)) != 0) {
                        ...
                        } else if (yCol > 0 && xCol < 0) // CSV, one column
                        {
                                if (!(r = config.addKey(prefix + ".dist.type", "discrete.csv.onecol")) ||
                                    !(r = config.addKey(prefix + ".dist.discrete.csv.onecol.file",
                                                        pDist->getFileName())) ||
                        ...
                        } else // CSV, two columns
                        {
                                if (!(r = config.addKey(prefix + ".dist.type", "discrete.csv.twocol")) ||
                                    !(r = config.addKey(prefix + ".dist.discrete.csv.twocol.file",
                                                        pDist->getFileName())) ||
                                                       ...
                                                       const std::string& prefix)
{
        ...
        if (distName == "fixed") {
                ...
                if (!(r = pDist0->init(densFileName, maskFileName, xOffset, yOffset, width, height, flipy, floor)))
                        abortWithMessage("Unable to initialize 2D discrete distribution for " + prefix + ": " +
                                         r.getErrorString());
        } else
