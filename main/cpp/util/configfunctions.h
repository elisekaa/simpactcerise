...
public:
        ...
        static void processConfigurations(
            ConfigSettings& config, GslRandomNumberGenerator* pRndGen,
            const std::vector<std::string>& excludeCategories = std::vector<std::string>());
        static void obtainConfigurations(
            ConfigWriter& config, const std::vector<std::string>& excludeCategories = std::vector<std::string>());
...
private:
        ...
        {
        public:
                ConfigFunctionsInternal(ProcessConfigFunction p = 0, ObtainConfigFunction o = 0,
                                        const std::string& n = "")
                    : procFunc(p), obtFunc(o), name(n)
                {
                }
                ConfigFunctionsInternal(const ConfigFunctionsInternal& src)
                    : procFunc(src.procFunc), obtFunc(src.obtFunc), name(src.name)
                {
                }
                ...
                bool operator()(const ConfigFunctionsInternal& a, const ConfigFunctionsInternal& b) const
                {
                        return (a.name < b.name);
                }
                void operator=(const ConfigFunctionsInternal& src)
                {
                        procFunc = src.procFunc;
                        obtFunc  = src.obtFunc;
                        name     = src.name;
                }
        };
