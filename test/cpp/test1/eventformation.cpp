...
EventFormation::EventFormation(Person* pPerson1, Person* pPerson2, double lastDissTime)
...
{
        return strprintf("Formation between %s and %s", getPerson(0)->getName().c_str(),
                         getPerson(1)->getName().c_str());
}
...
void EventFormation::fire(Algorithm* pAlgorithm, State* pState, double t)
{
        ...
        if (m_lastDissolutionTime >= 0) {
                std::cerr << "New formation between " << pPerson1->getName() << " and " << pPerson2->getName()
                          << " after " << (t - m_lastDissolutionTime) << " years" << std::endl;
        }
}
